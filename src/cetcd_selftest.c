/*  =========================================================================
    cetcd_selftest.c - run selftests
...
{
    test_item_t *item;
    printf ("Running cetcd selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (strcmp (item->testname, "private_classes") == 0)
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef CETCD_BUILD_DRAFT_API // selftest is still in draft
        else
            cetcd_private_selftest (verbose, item->subtest);
...
main (int argc, char **argv)
{
    ...
    if (test) {
        printf ("Running cetcd test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef CETCD_BUILD_DRAFT_API // selftest is still in draft
        else
            cetcd_private_selftest (verbose, test->subtest);
#endif // CETCD_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
